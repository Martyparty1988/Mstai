...
const FloatingActionButton: React.FC<FloatingActionButtonProps> = ({ onClick, variant = 'add' }) => {
  return (
    <button
      onClick={onClick}
      className="fixed bottom-24 right-6 md:bottom-12 md:right-12 z-50 p-5 rounded-full bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-accent)] text-white shadow-2xl hover:scale-110 active:scale-90 transition-all border border-white/20 animate-float group"
