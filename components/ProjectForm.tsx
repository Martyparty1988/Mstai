...
import type { Project, ProjectComponent, SolarTable } from '../types';
...
const ProjectForm: React.FC<ProjectFormProps> = ({ project, onClose }) => {
  ...
  useEffect(() => {
    if (project) {
      ...
      const fetchRelatedData = async () => {
        ...
        const existingTables = await db.solarTables.where('projectId').equals(project.id!).toArray();
        setTableList(existingTables.map(t => t.tableCode).join('\n'));
      };
      fetchRelatedData();
    }
  }, [project]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const manualTables = tableList
        .split(/[\n,]+/)
        .map(t => t.trim())
        .filter(t => t.length > 0)
        .map(code => {
            let type: 'small' | 'medium' | 'large' = 'small';
            if (code.toLowerCase().startsWith('it28')) type = 'small';
            if (code.toLowerCase().startsWith('it42')) type = 'medium';
            if (code.toLowerCase().startsWith('it56')) type = 'large';
            return {
                tableCode: code,
                tableType: type,
    ...
    try {
        ...
        if (projectId) {
            ...
            if (manualTables.length > 0) {
                 for (const t of manualTables) {
                     const exists = await db.solarTables.where({ projectId, tableCode: t.tableCode }).first();
                     if (!exists) {
                         await db.solarTables.add({ ...t, projectId });
                     }
                 }
            }
        } else {
            projectId = await db.projects.add(projectData as Project);
             ...
             if (manualTables.length > 0) {
                 await db.solarTables.bulkAdd(manualTables.map(t => ({ ...t, projectId: projectId! })));
             }
        }
        onClose();
    } catch (error) {
